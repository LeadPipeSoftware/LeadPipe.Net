// --------------------------------------------------------------------------------------------------------------------
// <copyright file="NoWhitespaceAttribute.cs" company="Lead Pipe Software">
...
{
    /// <summary>
	/// A custom data validation attribute that checks for whitespace.
	/// </summary>
	public class NoWhitespaceAttribute : LeadPipeValidationAttribute
	{
		/// <summary>
		/// Initializes a new instance of the <see cref="NoWhitespaceAttribute"/> class.
		/// </summary>
		/// <param name="ignoreIfConverted">if set to <c>true</c> [ignore if converted].</param>
		public NoWhitespaceAttribute(bool ignoreIfConverted = false) : base(ignoreIfConverted)
		{
		}
		...
		protected override ValidationResult PerformCustomValidation(object value, ValidationContext validationContext)
		{
			...
			if (convertedValue.ContainsWhiteSpace())
			{
				this.ErrorMessage = validationContext.DisplayName.FormattedWith(ValidationMessages.CanNotContainWhitespace);
