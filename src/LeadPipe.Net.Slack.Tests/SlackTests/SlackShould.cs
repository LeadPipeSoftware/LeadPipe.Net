...
namespace LeadPipe.Net.Slack.Tests.SlackTests
{
    ...
    public class SlackShould
    {
        ...
        /// </summary>
        [Test]
        public void ReturnBuiltUpMessageWithMultipleFields()
        {
            // Arrange
            Bootstrapper.Start();
            var slack = Bootstrapper.Container.GetInstance<ISlack>();
            ...
            var expectedFields = new List<SlackMessageAttachmentField>
            {
                new SlackMessageAttachmentField("title-0", "value-0", false),
                new SlackMessageAttachmentField("title-1", "value-1", true)
            };
            ...
            var message = slack.Send.MessageWithAttachment
                                        ...
                                        .WithTitleLink(expectedTitleLink)
                                        .WithText(expectedText)
                                        .IncludingFields(expectedFields)
                                        .WithImageUrl(expectedImageUrl)
                                        .WithThumbUrl(expectedThumbUrl)
                                        .WithFooter(expectedFooter)
                                        .WithFooterIcon(expectedFooterIcon)
            ...
            var firstField = message.Attachments.First().Fields.OrderByDescending(f => f.Title).Last();
            Assert.AreEqual(firstField.Title, "title-0");
            Assert.AreEqual(firstField.Value, "value-0");
            Assert.AreEqual(firstField.IsShort, false);

            var secondField = message.Attachments.First().Fields.OrderByDescending(f => f.Title).First();
            Assert.AreEqual(secondField.Title, "title-1");
            Assert.AreEqual(secondField.Value, "value-1");
            Assert.AreEqual(secondField.IsShort, true);
        }
